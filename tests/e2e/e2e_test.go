...
var _ = Describe("odoe2e", func() {
	...
	Context("odo project", func() {
		...
		It("should list the project tmp", func() {
                        listProj1 := runCmd("odo project list")
                        Expect(listProj1).To(ContainSubstring(projName))
                })

		// project list fails on travis 
		It("should list the project", func() {
			listProj := runCmd("odo project list -v")
